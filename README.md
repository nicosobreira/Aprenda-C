# Aprenda C

## Introdução

Esse repositório contém todo o básico de C.

Aqui estão algumas recomendações de como usar o material.

### Recomendações

- Não copie e cole o código desse tutorial. Digite cada linha, cada letra de cada código.
- Não use Inteligência Artificial, apenas se recomendado. Prefira os sites em [Referência](referência) e tutoriais no [Youtube](https://www.youtube.com).

## Conteúdos

1. [Olá, Mundo](./intro/README.md)
2. [Variáveis Primitivas](./variaveis/primitivas.md)
3. [Escopos](./variaveis/escopos.md)
3. [Condicionais](./condicional/README.md)
4. [Estrutura de Repetição](./repeticao/README.md)
5. [Projeto 1](./projeto/um.md)
6. [Ponteiros](./ponteiros.md)
7. [Listas](./variaveis/lista.md)
8. [Matrizes](./variaveis/matriz.md)
9. [Strings](./variaveis/string.md)
10. [Structs](./variaveis/struct.md)
11. [Enums](./variaveis/enum.md)
12. [Funções](./funcoes/README.md)
13. [Projeto 2](./projeto/dois.md)
14. [Compilador](./arquivos/compilador.md)
15. [Múltiplos arquivos](./arquivos/header.md)
16. [Projeto 3](./projeto/tres.md)
<!-- 17. [Build System]() -->

## Referência

- [Geeks for Geeks](https://www.geeksforgeeks.org/) para o funcionamento da linguagem.
- [DevDocs](https://devdocs.io/c/) para as funções da biblioteca padrão (std).
- [Stack Overflow](https://stackoverflow.com/questions) para dúvidas mais especificas. Pode digitar a dúvida no google que esse site provavelmente já aparece.

